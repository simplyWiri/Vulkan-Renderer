...
namespace Renderer::RenderGraph
{
	...
	public:
		...
		public:
			...
			PassDesc& ReadImage(const std::string& name, VkPipelineStageFlags flags, VkAccessFlags access, VkImageLayout expectedLayout = VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL_KHR); // Syncrhonisation2
			...
			PassDesc& SetRecordFunc(std::function<void(VkCommandBuffer, const FrameInfo&, GraphContext& context)> func) { this->execute = std::move(func); return *this; }
			PassDesc& SetInitialisationFunc(std::function<void(RenderGraph*)> func) { this->initialise = std::move(func); return *this; }
		};
