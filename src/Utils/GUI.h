	...
	class GUI
	{
		...
		public:
			...
			void initialise(Core* core)
			{
				...
				auto stagingBuffer = new Buffer(core->GetAllocator(), texWidth * texHeight * 4 * sizeof(char), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
				...
				for (int i = 0; i < 3; i++) { buffers.push_back(new Buffer(core->GetAllocator(), 1024 * sizeof(float), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU)); }
			}

			void updateBuffers(ImDrawData* imDrawData, uint32_t offset)
			{
				if (imDrawData->CmdListsCount == 0) return;

				auto* buffer = buffers[offset];

				if (buffer->getSize() == 0 || buffer->getSize() < imDrawData->TotalVtxCount * sizeof(ImDrawVert) + imDrawData->TotalIdxCount * sizeof(ImDrawIdx))
				{
					buffer->reSize(imDrawData->TotalVtxCount * sizeof(ImDrawVert) + imDrawData->TotalIdxCount * sizeof(ImDrawIdx));
				}

				auto vtxDst = buffer->map();
			...
			void AddToGraph(Rendergraph* graph)
			{
				graph->AddPass(PassDesc().SetName("ImGui-CPU").SetRecordFunc([&](VkCommandBuffer buffer, const FrameInfo& info, GraphContext& context)
				{
					...
					io.MousePos = ImVec2(static_cast<float>(mouseX), static_cast<float>(mouseX));
					...
					ImGui::Text("%.2f ms/frame (%.1d fps)", (1000.0f / info.fps), info.fps);
					...
					updateBuffers(drawData, info.offset);

					// I should probably be buffering these...
				...
				graph->AddPass(
					PassDesc().SetName("ImGui-Render").SetInitialisationFunc([&](Tether& tether) { tether.GetDescriptorCache()->writeSampler(key, "sTexture", fontImage, sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL); }).SetRecordFunc(
						[&](VkCommandBuffer buffer, const FrameInfo& info, GraphContext& context)
						{
							...
							vkCmdBindVertexBuffers(buffer, 0, 1, &buffers[info.offset]->getBuffer(), offsets);
							vkCmdBindIndexBuffer(buffer, buffers[info.offset]->getBuffer(), imDrawData->TotalVtxCount * sizeof(ImDrawVert), VK_INDEX_TYPE_UINT16);
