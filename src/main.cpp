		...
		.SetRecordFunc([tribuffer, program](VkCommandBuffer buffer, const FrameInfo& frameInfo, GraphContext& context) -> void
			{
				...
				context.graph->getGraphicsPipelineCache().bindGraphicsPipeline(buffer, context.getDefaultRenderpass(), context.getExtent(), DepthSettings::DepthTest(), { BlendSettings::Add() }, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, program);
